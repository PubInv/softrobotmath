...
My precursor strategy now is to conpute $r_b$ direction from $\theta_{ab}$:
\begin{align}
  U_x &= r_a/\sin{\theta_{ab}} \\
  H_y = U_x \tan{\theta_{ab}} \\
  r_b &= \frac{(U_x - r_a)\sin{\theta_{ab}}}{\sin{\theta_{ab}}+1}
  Z' = \frac{H_y}{\sin{\gamma}}
\end{align}


\section{Idea as of Sept. 12th}

\newcommand{\abs}[1]{ \left\lvert#1\right\rvert}

After failing with a different idea, I now imagine the following
strategy.

Having computed the top plane, we new the point $C$ has
a perpendicular distanct of $c$ to this plane.
All such points form a line parallel to the line $UZ$.
We will first compute the equation of this line.

Then we know that the point $C$ but have distance $a+c$ from
the point $A$ (the origin) and distance $b+c$ from the known
point $B$. This gives us three equations, which
should be more than enough to solve for the point $C$.

Let $H = [0, H_y,0]$.
Let $\iota$ be the angle of the tilted plane perpendicular to $\overline{UZ'}$.
...
\iota &= \arccos{\abs{\frac{\overrightarrow{n_1} \cdot \overrightarrow{N}}
  {\abs{\overrightarrow{n_1}} \abs{\overrightarrow{N}}}}} \\
\end{align}
...
The equation of our plane is $ \overrightarrow{N} \cdot \overrightarrow{X} = k$
\begin{align}
  k &= \frac{N \cdot U}{N_y}
c &= \frac{\abs{N \cdot P - k}}{\abs{N}} \\
c &= \frac{\abs{C_x * N_x  + C_z * N_z - k}}{1} \\
\end{align}

This much has been tested in code. This last equation
is a convenient line forming a system of equations with
the curve generated by a circle touching both $A$ and $B$.
Solving this system of equations to find the intersection
of the line and the curve should give us the coordinates
of $C$, and therefore complete our computation.
...
This math has been checked in Javascript and it appearts to be correct in computing c,
though it is sometimes off in the second digit, which I assume is a numerical problem
