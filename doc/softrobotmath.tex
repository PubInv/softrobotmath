...
by a rotation $\gamma$ of the $[1,0,0]$ vector about the $Z$-axis followed by a rotation $\theta$
about the $X$-axis.
...
\begin{align}
  ...
  \psi &= \text{angle of CA against the z axis} \\
  L &= \| A - CA_{apex} \| \\
  Z' &= L \cos{\psi} \frac{AB_{apex}[x]}{AB_{apex}[x]-CA_{apex}[x]} \\
  \gamma &= \frac{\pi}{2} - \theta_{ab} \\
  \theta &= \frac{\pi}{2} - \arccos{\frac{r_a}{Z'}};
\end{align}
